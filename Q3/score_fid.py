...
def calculate_fid_score(sample_feature_iterator,
    ...
    # Features extracted from generated samples and testset, 
	# using CNN layers from classifier.py
    sample_features = []
    testset_features = []
	# Obtain all features from 1000 samples for statistic calculation
    for batch_idx, features in enumerate(sample_feature_iterator):
        sample_features.append(features)
    for batch_idx, features in enumerate(testset_feature_iterator):
        testset_features.append(features) 
    # Statistic calculation from generated samples and testset    
    mu_q = np.mean(sample_features, axis=0)
    sigma_q = np.cov(sample_features, rowvar=False)
    mu_p = np.mean(testset_features, axis=0)  
    sigma_p = np.cov(testset_features, rowvar=False)
    # Using the FID calculation formula
    diff = mu_p- mu_q
    #sigma_mean = linalg.sqrtm(sigma_p.dot(sigma_q), disp=False) 
	# For stability
    offset = np.eye(sigma_p.shape[0]) * 1e-6 
    sigma_mean = linalg.sqrtm((sigma_p + offset).dot(sigma_q + offset))
    sigma_mean = sigma_mean.real 
    tr_sigma_mean = np.trace(sigma_mean)
	# Finally return the score
    fid = diff.dot(diff) + np.trace(sigma_p) + np.trace(sigma_q) - 2 * tr_sigma_mean
    return fid
...
if __name__ == "__main__":
    ...
    parser.add_argument('directory', type=str, default="fid_samples",
